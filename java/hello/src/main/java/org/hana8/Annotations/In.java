...
public @interface In {
	...
	class Validate {
		public static String validate(Field f, Object fval) {
			...
			Function<Annotation, String> msgFn = a -> String.format(((In)a).msg(), Arrays.toString(value));

			return Validator.validate(f, fval, In.class, msgFn, () -> List.of(value).contains(v));
			// In annotation = f.getAnnotation(In.class);
			// String v = String.valueOf(fval);
			// if (!List.of(annotation.value()).contains(v))
			// 	return null;
			// return String.format(annotation.msg(), Arrays.toString(annotation.value()));
		}
	}
}
